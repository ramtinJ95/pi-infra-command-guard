...
`integrations.typesafe` is an experimental, advisory-only review and defaults to `enabled: false`. When a user asks to turn it on or off, prefer telling them to run `/infra-guard-typesafe enable` or `disable`, which edit only that field; editing the JSON directly is equivalent. It requires `TYPESAFE_API_KEY` in Pi's environment and sends the blocked command and guard reason, both credential-redacted, to TypeSafe only after a block based on a positively recognized risk. Direct JSON edits are observed on the next configuration read (the next shell command, approval request, or slash command), not instantly; the file is not watched. Explain that the verdict never changes a block or approval, that pauses via `/infra-guard-typesafe pause` are session-only, and that `timeoutMs` must be an integer between 1000 and 30000. Do not add the field for users who have not asked for it.
...
- The TypeSafe review is advisory only. It runs solely after `guardExecution` returns a pending block with `basis: "knownRisk"` in TUI mode, never for allowed, unclassified, custom-rule, interactive-TTY, or non-TUI decisions, and never on the approved retry. It must never change a decision, create or consume an approval, or alter the approval binding. Render it in its own labelled overlay section, keep failures visible, and never inject TypeSafe requests into the every-command path. `enable`/`disable` persist through `persistTypeSafeEnabled`; the review pause is session-only like guard pauses. Redact credential material from both the command and the reason before sending, on complete shell words with a shell-safe placeholder, and refuse to send a command whose quoting cannot be resolved rather than guess. Keep the exact originals for cache identity and the approval binding. Sanitize and bound every string TypeSafe supplies before it reaches the overlay or a notification. Honour the approval tool's abort signal before notifying or opening the overlay. Persist configuration atomically through a temporary sibling that preserves permissions. Mock the transport in tests; never send live commands or make paid requests from the test suite.
