...
test("approval attention is silent by default and requires explicit opt-in", () => {
	assert.deepEqual(approvalAttentionSettings({}), {
		customSoundPath: undefined,
		nativeNotification: false,
	});
	assert.deepEqual(
		approvalAttentionSettings({
			PI_INFRA_COMMAND_GUARD_SOUND_PATH: "/tmp/approval.wav",
			PI_INFRA_COMMAND_GUARD_NATIVE_NOTIFICATION: "yes",
		}),
		{
			customSoundPath: "/tmp/approval.wav",
			nativeNotification: true,
		},
	);
	assert.equal(
		approvalAttentionSettings({ PI_INFRA_COMMAND_GUARD_NATIVE_NOTIFICATION: "disabled" }).nativeNotification,
		false,
	);
});
